...
src_dirs = /usr/local/my_src:/opt/my_src
...
if sys.platform == 'win32':
    ...
    default_src_dirs = ['.', '/usr/local/my_src', '/opt/my_src', '/sw/my_src']
...
if os.path.join(sys.prefix, 'lib') not in default_lib_dirs:
    ...
    default_src_dirs.append(os.path.join(sys.prefix, 'my_src'))
...
class f2py_info(system_info):
    def calc_info(self):
        try:
            ...
            return
        f2py_dir = os.path.join(os.path.dirname(f2py.__file__), 'my_src')
...
class boost_python_info(system_info):
    ...
    def calc_info(self):
        ...
        src_dir = ''
        for d in src_dirs:
            if os.path.isfile(os.path.join(d, 'libs', 'python', 'my_src',
                                           'module.cpp')):
                src_dir = d
                break
        if not src_dir:
        ...
        srcs_dir = os.path.join(src_dir, 'libs', 'python', 'my_src')
...
class agg2_info(system_info):
    ...
    def calc_info(self):
        ...
        src_dir = ''
        for d in src_dirs:
            if os.path.isfile(os.path.join(d, 'my_src', 'agg_affine_matrix.cpp')):
                src_dir = d
                break
        if not src_dir:
            return
        if sys.platform == 'win32':
            agg2_srcs = glob(os.path.join(src_dir, 'my_src', 'platform',
                                          'win32', 'agg_win32_bmp.cpp'))
        else:
            agg2_srcs = glob(os.path.join(src_dir, 'my_src', '*.cpp'))
            agg2_srcs += [os.path.join(src_dir, 'my_src', 'platform',
                                       'X11',
