...
#         :param d:
#         """
#         # one pair has to be parallel
#         # how to use tools in primitives too? or is it prohibited?
#         vertices = [a,b,c,d]
#         vectors = []
#         for i in range(3):
#             vec = [b-a for a,b in zip(vertices[i], vertices[i+1])]
#             vectors.append(np.array(vec))
#         dots = np.dot(vertices[0],vertices[2]), np.dot(vertices[1],vertices[3])
#         if not any([np.isclose(dots[0],-1,atol=DEF_TOLERANCE), np.isclose(dot[1],-1,atol=DEF_TOLERANCE)]):
#             raise
